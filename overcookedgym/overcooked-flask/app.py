...
def load_policy(policy_path, algo_name):
    algo_name = algo_name.upper()
    if algo_name not in MODEL_LOADERS:
        raise ValueError(f"Unsupported algorithm for visualization: {algo_name}")
    return MODEL_LOADERS[algo_name].load(policy_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo_p0', type=str, default='PPO',
                        choices=sorted(MODEL_LOADERS.keys()),
                        help="algorithm used by player 0 model")
    parser.add_argument('--algo_p1', type=str, default='PPO',
                        choices=sorted(MODEL_LOADERS.keys()),
                        help="algorithm used by player 1 model")
    ...
    if ARGS.replay_traj:
        ...
        if ARGS.modelpath_p0:
            POLICY_P0 = load_policy(ARGS.modelpath_p0, ARGS.algo_p0)
        if ARGS.modelpath_p1:
            POLICY_P1 = load_policy(ARGS.modelpath_p1, ARGS.algo_p1)
