...
class OffPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if not self.algo_args["render"]["use_render"]:
            ...
            self.train_log_file = open(
                os.path.join(self.run_dir, "train_progress.txt"),
                "w",
                encoding="utf-8",
            )
    ...
    def run(self):
        ...
        for step in range(1, steps + 1):
            ...
            if step % self.algo_args["train"]["eval_interval"] == 0:
                ...
                self.log_train_progress(cur_step)
    ...
    def log_train_progress(self, step):
        """Log off-policy training rewards for stdout, tensorboard, and file."""
        average_step_reward = float(self.buffer.get_mean_rewards())
        print(
            f"Env {self.args['env']} Task {self.task_name} Algo {self.args['algo']} Exp {self.args['exp_name']} Step {step} / {self.algo_args['train']['num_env_steps']}, average step reward in buffer: {average_step_reward}.\n"
        )
        self.writter.add_scalar(
            "train_average_step_rewards", average_step_reward, step
        )

        average_episode_reward = np.nan
        if len(self.done_episodes_rewards) > 0:
            average_episode_reward = float(np.mean(self.done_episodes_rewards))
            print(
            ...
            self.writter.add_scalars(
                ...
                {"aver_rewards": average_episode_reward},
                step,
            )
            self.done_episodes_rewards = []

        self.train_log_file.write(
            ",".join(map(str, [step, average_step_reward, average_episode_reward]))
            + "\n"
        )
