...
class HAD3QN(OffPolicyBase):
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        ...
        self.epsilon_start = args.get("epsilon_start", self.epsilon)
        self.epsilon_finish = args.get("epsilon_finish", self.epsilon)
        self.epsilon_anneal_steps = max(1, int(args.get("epsilon_anneal_steps", 1)))
        self.epsilon_step = 0
    ...
    def get_actions(self, obs, epsilon_greedy):
        ...
        epsilon = self._get_epsilon(epsilon_greedy)
        if np.random.random() < epsilon and epsilon_greedy:
    ...
    def _get_epsilon(self, epsilon_greedy):
        """Return the current epsilon and optionally advance the annealing schedule."""
        if not epsilon_greedy:
            return self.epsilon_finish

        if self.epsilon_start == self.epsilon_finish:
            return self.epsilon

        fraction = min(1.0, self.epsilon_step / self.epsilon_anneal_steps)
        epsilon = self.epsilon_start + fraction * (
            self.epsilon_finish - self.epsilon_start
        )
        self.epsilon_step += 1
