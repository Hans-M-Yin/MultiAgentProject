...
class AdapPolicy(ActorCriticPolicy):
    ...
    def _append_context_to_features(
        self,
        features: th.Tensor,
        context: Optional[th.Tensor] = None,
    ) -> th.Tensor:
        """
        Concatenate the latent context to extracted observation features.

        When collecting rollouts, the policy receives raw environment
        observations and should use the policy's current context. During
        training, the rollout buffer already stores observations with the
        context appended, and callers can pass that explicit context instead.
        """
        if context is None:
        ...
        if context.size(0) == 1:
            context = context.repeat(features.size(0), 1)
        return th.cat((features, context), dim=1)
    ...
    def _get_latent(self,
        ...
        features = self._append_context_to_features(features)
        latent_pi, latent_vf = self.mlp_extractor(features)

        # Features for sde
        latent_sde = latent_pi
        sde_features_extractor = getattr(self, "sde_features_extractor", None)
        if sde_features_extractor is not None:
            latent_sde = sde_features_extractor(features)
    ...
    def forward(
        ...
        """
        SB3 >= 1.7 bypasses `_get_latent()` in `ActorCriticPolicy.forward()`.
        Override the full forward pass so the context is always concatenated.
        """
        features = self.extract_features(obs)
        features = self._append_context_to_features(features)
        latent_pi, latent_vf = self.mlp_extractor(features)
    ...
    def get_distribution(self, obs: th.Tensor):
        """
        Build the action distribution from raw observations plus current
        context, matching the custom forward pass above.
        """
    ...
    def predict_values(self, obs: th.Tensor) -> th.Tensor:
        """
        Predict state values from raw observations plus current context.
        """
    ...
    def evaluate_actions(self,
        ...
        features = self._append_context_to_features(
