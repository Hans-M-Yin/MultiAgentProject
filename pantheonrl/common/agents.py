...
class StaticModelAgent(Agent):
    """
    Agent representing a frozen SB3 model.

    Unlike ``StaticPolicyAgent``, this wrapper uses the algorithm's
    ``predict()`` path directly, which lets fixed off-policy agents such as
    DQN/SAC be reused without depending on actor-critic policy APIs.

    :param model: Loaded SB3 algorithm instance
    """
    ...
    def get_action(self, obs: Observation, record: bool = True) -> np.ndarray:
        ...
        return clip_actions(np.asarray(action), self.model)
...
class OffPolicyAgent(Agent):
    ...
    def update(self, reward: float, done: bool) -> None:
        ...
        # Off-policy algorithms should not train before enough replay data has
        # been collected. SB3's native learn loop handles this check, but this
        # wrapper needs to mirror it explicitly.
